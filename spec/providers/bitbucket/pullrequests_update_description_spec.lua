...
describe("bitbucket pullrequests.update_description", function()
	local calls
	local cache_cleared
	...
	it("PUTs the new description as JSON to the PR's self link", function()
		package.preload["atlas.pulls.providers.bitbucket.api.service"] = function()
			return {
		...
		assert.equal('{"description":"New body"}', calls[1].body)
