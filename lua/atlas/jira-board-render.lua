...
local ui = require("atlas.jira-board-ui")
...
local function render_issue_line(node, depth, row)
  ...
  local star = ""
  if node.key and state.favorites and state.favorites[node.key] then
    star = " ★"
  end
  ...
  -- LEFT --------------------------------------------------
  local left = ("%s%s %s %s %s%s %s"):format(indent, expand_icon, icon, key, title, star, pts)
  ...
  if star ~= "" then
    add_hl(highlights, col, star, "Special")
    col = col + #star + 1
  end
...
local function render_header(view)
  ...
  table.insert(left_tabs, { name = "JQL", key = "J" })
