...
function M.groups(session, actions, buf, opts)
	...
	if include_actions then
		...
		add(review, "ui.open_actions", "Review actions", 7, actions.open_actions)
	...
	if include_actions and content then
		...
		add_range_action("pulls.review.diff.submit_comment", "Submit inline comment", 31, function(start_line, end_line)
		...
		add(review, "pulls.review.diff.delete", "Delete comment or note at cursor", 34, function()
			if not notes.delete_at_cursor(session, buf) then
				actions.delete_comment(buf)
			end
		end)
		if new_side then
			add(review, "pulls.review.diff.add_note", "Add local note", 35, function()
