...
local repositories = require("atlas.pulls.providers.forgejo.api.repositories")
local search = require("atlas.pulls.providers.forgejo.completion.search")
		...
		local function resolve_user(current)
			...
			pullrequests.fetch_user(function(current_user, err)
				...
				if err or username == "" then
					local message = tostring(err or "Missing current Forgejo user")
