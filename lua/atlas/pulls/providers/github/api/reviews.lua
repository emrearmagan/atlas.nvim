...
---@return { cancel: fun() }|nil
local function current(pr, on_done)
...
query($owner:String!,$name:String!,$number:Int!){
  repository(owner:$owner,name:$name){
    pullRequest(number:$number){viewerLatestReview{databaseId state}}
  }
}
]]
	return cli.gh({
...
---@param body string
---@param on_done fun(ok: boolean, err: string|nil)
local function dismiss(pr, review, repo_slug, body, on_done)
...
function M.toggle_approval(pr, on_done)
	return current(pr, function(review, repo_slug, err)
		if err then
			on_done(nil, err)
			return
		end
		...
		if review and review.databaseId and (state == "APPROVED" or state == "CHANGES_REQUESTED") then
			dismiss(pr, review, repo_slug, "", function(ok, dismiss_err)
				if not ok then
		...
		M.approve(pr, nil, "", function(ok, approve_err)
