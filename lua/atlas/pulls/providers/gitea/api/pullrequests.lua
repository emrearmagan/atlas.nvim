...
local pagination = require("atlas.pulls.providers.gitea.api.pagination")
local mapper = require("atlas.pulls.providers.gitea.api.mapper")
local request_scope = require("atlas.core.requests")

local M = {}
local cache_namespace = "gitea"

local function draft_prefix()
	local prefix = vim.trim(config.domain_options("gitea", "pulls").draft_prefix or "")
...
local function cache_scope()
	return string.format("%s:pulls:%s", cache_namespace, service.base_url())
end
...
local function patch_title(pr, title, on_done)
	...
	if not endpoint or title == "" then
		on_done(false, endpoint and "Title cannot be empty" or "Invalid Gitea repository")
		return nil
	end
...
function M.update_title(pr, title, on_done)
	...
	return patch_title_checked(
		...
		draft,
		"Gitea did not preserve the draft state; configure an enabled draft_prefix",
		on_done
	)
end
...
function M.set_draft(pr, draft, on_done)
	...
	return patch_title_checked(
		...
		with_draft_state(title, draft),
		draft,
		"Gitea did not change the draft state; configure an enabled draft_prefix",
		on_done
	)
end
...
function M.set_subscription(pr, username, subscribed, on_done)
	...
	if not endpoint or username == "" or not tostring(pr.id):match("^%d+$") then
		on_done(false, "Invalid Gitea subscription")
		return nil
	end
	local target =
...
function M.set_state(pr, state, on_done)
	...
	if not endpoint or (state ~= "open" and state ~= "closed") then
		on_done(nil, endpoint and "Invalid pull request state" or "Invalid Gitea repository")
		return nil
	end
