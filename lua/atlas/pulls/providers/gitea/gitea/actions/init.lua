...
local registry = require("atlas.pulls.providers.gitea.gitea.actions.registry")
local statusline = require("atlas.ui.statusline")

---@alias AtlasGiteaActionId
---| AtlasPullActionId
---| "reopen"
---| "update_branch"
---| "edit_assignees"
...
function M.run(id, ctx, on_done)
	...
	if action == nil then
		...
		logger.logerror("gitea.pulls.action.unknown", { action_id = tostring(id) })
		on_done(nil, err)
		return false
	end
	...
	if not available then
		...
		logger.logwarn("gitea.pulls.action.unavailable", { action_id = tostring(id), error = err })
