...
local pipeline_api = require("atlas.pulls.providers.gitea.gitea.api.pipelines")
...
local function fetch_branch_protection(pr, on_done)
	...
	active = service.request("GET", base .. "/branches/" .. service.url_encode(branch), nil, function(raw, err)
		...
		if type(raw) ~= "table" then
			on_done(nil, "Invalid Gitea branch response")
			return
		end
		...
		active = service.request(
			...
			function(value, rule_err)
				...
				elseif type(value) ~= "table" then
					on_done(nil, "Invalid Gitea branch protection response")
				else
					on_done(value, nil)
				end
			end
...
local function aggregate_status(values)
	...
	for _, value in ipairs(values) do
		...
		if STATUS_PRIORITY[value] ~= nil and value ~= "skipped" then
