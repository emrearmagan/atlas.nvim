...
function M.state(value, conclusion)
	local state = (json.safe_str(conclusion) or ""):lower()
	if state == "" then
		state = (json.safe_str(value) or ""):lower()
	end
...
local function fetch_statuses(endpoint, on_done)
	...
	return pagination.fetch_all(endpoint, { sort = "leastindex" }, {
		invalid_response = "Invalid Gitea commit status response",
		accept = function(raw)
		...
		if type(raw) == "table" then
			...
			if not run_id then
				...
				table.insert(pipeline.jobs, {
					id = tonumber(job_id) or raw.id or target_url or (#pipeline.jobs + 1),
...
function M.fetch(pr, _opts, on_done)
	...
	if not endpoint then
		on_done(sha == "" and {} or nil, sha == "" and nil or "Invalid Gitea repository")
		return nil
	end
...
local function timestamp(value)
	local result = json.safe_str(value)
	if
		result == nil
		or result == ""
...
local function duration(started_at, completed_at)
	if started_at == nil or completed_at == nil or type(vim.fn.strptime) ~= "function" then
		return nil
	end
...
local function provider_state(raw)
	local conclusion = json.safe_str(raw.conclusion) or ""
	if conclusion ~= "" then
		return conclusion
	end
	return json.safe_str(raw.status) or ""
end
...
local function map_step(raw, job_id, index)
	local started_at = timestamp(raw.started_at)
	local completed_at = timestamp(raw.completed_at)
	return {
		id = string.format("%s:%s", tostring(job_id), tostring(raw.number or index)),
		name = json.safe_str(raw.name) or "Step",
...
---@param raw table
---@return PullsPipelineJob[]|nil, string|nil
local function map_jobs(raw)
	local values = json.nilify(raw.jobs)
	if type(values) ~= "table" then
		return nil, "Invalid Gitea Actions jobs response"
	end
	...
	for _, value in ipairs(values) do
		...
		local steps = {}
		for index, step in ipairs(json.safe_table(value.steps)) do
			if type(step) == "table" then
				table.insert(steps, map_step(step, value.id, index))
			end
		end
		table.insert(jobs, {
			id = value.id,
			name = json.safe_str(value.name) or ("Job #" .. tostring(value.id)),
			state = M.state(value.status, value.conclusion),
			provider_state = provider_state(value),
			url = service.absolute_url(json.safe_str(value.html_url)),
			started_at = started_at,
			completed_at = completed_at,
			duration = duration(started_at, completed_at),
			steps = steps,
	...
	requests.run(function(done)
		...
		requests.run(function(done)
			...
			local result = vim.tbl_extend("force", {}, pipeline, {
				...
				commit_hash = json.safe_str(run.head_sha) or pipeline.commit_hash,
				jobs = jobs,
			})
			on_done(result, nil)
		end)
...
function M.fetch_job_log(pr, pipeline, job, on_done)
	local base = repo_endpoint(pr)
	local job_id = type(job) == "table" and tonumber(job.id) or nil
