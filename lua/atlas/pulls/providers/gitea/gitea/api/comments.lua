					...
					starts[tostring(index)] = function(done)
						...
						if comment.id == "__body__" then
							return pagination.fetch_all(target, nil, {
								invalid_response = "Invalid Gitea reactions response",
							}, mapped)
						end
...
function M.add(pr, content, opts, on_done)
	...
	if parent and parent.inline then
		local root_id = tostring(parent.parent_id or parent.id or "")
		...
		return service.request(
			...
			function(raw, err)
				...
				if created == nil then
					on_done(nil, "Invalid review comment response")
					return
				end
...
function M.set_thread_resolved(pr, root, resolved, on_done)
	local base = endpoint(pr)
	local root_id = type(root) == "table" and tostring(root.parent_id or root.id or "") or ""
	if not base or not root_id:match("^%d+$") or type(root.inline) ~= "table" then
