...
local function run(query)
	...
	if layout.is_open() and state.provider and state.provider.id == "forgejo" then
		require("atlas.issues.ui.main.controller").switch_view(view)
		return
	end

	require("atlas").open("issues", "forgejo", { initial_view = view })
end

---@param default string|nil
function M.open(default)
	require("atlas.commands.search.prompt").open({
		name = "AtlasForgejoIssueSearch",
		on_submit = run,
		default = default,
	})
end
