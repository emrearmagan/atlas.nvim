...
local function build_autocomplete_index(data)
	...
	for _, fn in ipairs(data.visibleFunctionNames or {}) do
		...
		local insert = vim.trim(value)
		if insert ~= "" and not insert:find("%(") then
			insert = insert .. "()"
		end
		local types = {}
...
local function complete_cmdline(arglead, cmdline, cursorpos)
	...
	if not field_meta then
		return cap(suggest_fields(input_prefix ~= "" and input_prefix or field_token))
	end
	...
	local matched_operator, matched_op_words = parse_operator_match(field_meta.operators, operator_words)
	local still_typing_operator = false
	if partial ~= "" and #operator_words >= matched_op_words then
		still_typing_operator = has_operator_prefix(field_meta.operators, operator_input)
	end
