...
local function with_details(issue, done, run)
	api.get(issue, {}, function(details, err)
		...
		---@cast details GiteaIssueDetails
		run(details)
	end)
end
...
local function create_repositories(ctx)
	...
	local options = config.domain_options("gitea", "issues") or {}
	local result, seen = {}, {}
	---@param value AtlasGiteaIssuesSearchConfig
	local function add(value)
		local configured = vim.trim(value.repo or "")
		if configured:match("^[^/%s]+/[^/%s]+$") and not seen[configured] then
			seen[configured] = true
			table.insert(result, configured)
		end
	end
...
local function set_locked(ctx, locked, reason, done)
	...
	api.set_locked(issue, locked, reason, function(ok, err)
		if not ok then
			done(nil, err)
			return
		end
