...
function M.header_rows(issue, details, loading)
	...
	local assignees = {}
	for _, value in ipairs(details and details.assignees or (data.assignee and { data.assignee } or {})) do
...
function M.fetch_header(issue, details, _opts, on_done)
	return require("atlas.issues.providers.gitea.api").issues.check_subscription(details.key, function(subscribed, err)
		if not err then
