...
---@return AtlasGiteaIssuesConfig
function M.gitea_config()
	local config = require("atlas.config")
	return ((config.options.issues or {}).providers or {}).gitea or {}
end
...
function M.tea(args, callback, ctx)
	...
	local cmd = vim.list_extend({ "tea" }, args)
	local log = vim.tbl_extend("keep", { cmd = table.concat(cmd, " ") }, ctx or {})
	local message = log.action or "Gitea CLI"
	log.action = nil
	...
	local handle = vim.system(cmd, { text = true }, function(res)
		vim.schedule(function()
			...
			if res.code ~= 0 then
				...
				logger.logerror("Gitea issues request failed", { code = res.code, err = err })
				callback(nil, err)
				return
			end
...
---@param body string|nil  JSON string or nil
---@param callback fun(result: any, err: string|nil)
---@param ctx table|nil
---@return { job_id: integer, cancel: fun() }|nil
...
---@param ctx table|nil
---@return { job_id: integer, cancel: fun() }|nil
function M.get(endpoint, callback, ctx)
	return M.tea({ "api", endpoint }, callback, ctx)
end
