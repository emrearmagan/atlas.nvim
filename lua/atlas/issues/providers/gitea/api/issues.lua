...
local function cache_scope()
	return string.format("gitea:issues:%s", service.base_url())
end
...
---@param value GiteaIssue|IssueRef|string
---@return string|nil, integer|nil, string|nil, string|nil
local function issue_endpoint(value)
	local slug, number
	if type(value) == "table" then
		---@cast value GiteaIssue
		slug, number = value.repo_full_name, value.number
		if not slug or not number then
			slug, number = mapper.parse_key(value.key)
		end
	else
		slug, number = mapper.parse_key(value)
	end
	local base = repo_endpoint(slug)
	if not base or not number then
		return nil, nil, nil, nil
	end
	local key = string.format("%s#%d", slug, number)
	return string.format("%s/issues/%d", base, number), number, slug, key
end
...
function M.fetch_by_refs(refs, _opts, on_done)
	...
	for index, ref in ipairs(refs) do
		local endpoint, _, slug = issue_endpoint(ref)
		if not endpoint then
			on_done({}, "Invalid Gitea issue key: " .. tostring(ref.key))
			return nil
		end
		starts[index] = function(done)
			return service.request("GET", endpoint, nil, function(raw, err)
				done(raw and mapper.to_issue(raw, slug) or nil, err)
			end)
		end
	end
...
function M.update(issue, changes, on_done)
	local endpoint, _, slug, key = issue_endpoint(issue)
	if not endpoint then
		on_done(nil, "Invalid Gitea issue key")
		return nil
	end
...
function M.update_issue(issue, changes, on_done)
	...
	return M.update(issue, payload, on_done)
end
...
---@param on_done fun(ok: boolean, err: string|nil)
function M.set_locked(issue, locked, reason, on_done)
	local endpoint, _, _, key = issue_endpoint(issue)
	if not endpoint then
		on_done(false, "Invalid Gitea issue key")
		return nil
