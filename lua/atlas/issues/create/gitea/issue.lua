...
local function pick_assignees(state)
	...
	state.requests.run(function(done)
		return api.list_assignees(state.fields.repo_slug, done)
...
local function pick_labels(state)
	...
	state.requests.run(function(done)
		return api.list_labels(state.fields.repo_slug, done)
	end, function(items, err)
		if err or not items then
...
local function pick_milestone(state)
	...
	state.requests.run(function(done)
		return api.list_milestones(state.fields.repo_slug, done)
	end, function(items, err)
		if err or not items then
...
local function submit(state)
	...
	state.requests.run(function(done)
		return api.update_issue(state.issue, {
			title = title,
			body = form.get_body(state.layout),
			assignees = assignees,
			...
			},
			{
				...
				action = function()
					...
					templates.open({
						...
						picker_kind = "atlas_gitea_templates",
						menu_kind = "atlas_gitea_templates_menu",
					})
				end,
			},
		},
