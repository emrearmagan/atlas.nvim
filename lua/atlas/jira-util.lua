...
---Build issue tree grouped by status
---@param issues table[]
---@return JiraIssueNode[]
function M.build_issue_tree_by_status(issues)
  -- Group issues by status category
  local status_groups = {}
  local status_order = {"To Do", "In Progress", "Done"}
  
  for _, issue in ipairs(issues) do
    local status_cat = issue.status_category or "To Do"
    if not status_groups[status_cat] then
      status_groups[status_cat] = {}
    end
    table.insert(status_groups[status_cat], issue)
  end
  
  -- Build tree for each status group
  local roots = {}
  for _, status_cat in ipairs(status_order) do
    local group_issues = status_groups[status_cat]
    if group_issues and #group_issues > 0 then
      -- Create status header node
      local status_node = {
        key = "STATUS_" .. status_cat,
        summary = status_cat,
        status = status_cat,
        type = "StatusGroup",
        children = M.build_issue_tree(group_issues),
        expanded = true,
        is_status_group = true,
      }
      table.insert(roots, status_node)
    end
  end
...
function M.build_issue_tree(issues)
  ...
  -- Create placeholder nodes for missing parents (Epics) - only if they don't exist
  local created_placeholders = {}
  for _, issue in ipairs(issues) do
    if issue.parent and not key_to_issue[issue.parent] then
      -- Create a placeholder Epic node (only if Epic wasn't fetched)
      -- Use parent_summary if available, otherwise just show the key
      local epic_summary = issue.parent_summary or (issue.parent .. " (Epic)")
      key_to_issue[issue.parent] = {
        key = issue.parent,
      ...
      table.insert(created_placeholders, issue.parent)
    end
  end
  ...
  -- Add placeholder Epics to roots (they have no parent themselves)
  for _, epic_key in ipairs(created_placeholders) do
    table.insert(roots, key_to_issue[epic_key])
  end
...
function M.setup_static_highlights()
  ...
  local hl = {
    ...
    -- Jira Details overlay highlights
    JiraDetailsHeader = { fg = "#61afef", bold = true },
    JiraDetailsFooter = { fg = "#abb2bf", italic = true },
    JiraDetailsLabel = { fg = "#c678dd", bold = true },
  }
