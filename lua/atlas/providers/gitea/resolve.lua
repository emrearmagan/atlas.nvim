...
local function repository(parsed, owner, repo)
	...
	local web_url = url.base_url("gitea", parsed.host, "gitea.com") .. "/" .. full_name
...
function M.resolve(value, parsed)
	...
	local base = url.configured_base("gitea")
	if base == nil then
		base = { host = "gitea.com", path = "", remote = false }
	end
	...
	if parsed.remote then
		...
		if owner == nil then
			return nil, "Unsupported Gitea remote. Expected owner/repository"
		end
	...
	if owner then
		if not url.valid_tail(tail) then
			return nil, "Unsupported Gitea URL"
		end
	...
	return nil, "Unsupported Gitea URL. Expected a repository, issue, or pull request URL"
end
