...
local clients = require("atlas.providers.gitlab.client")
...
function M.new(domain)
	...
	function api.fetch(opts, on_done)
		...
		local cache_key = string.format("gitlab:%s:todos:state=%s:per_page=%d", domain, state, per_page)
	...
	---@return { cancel: fun() }|nil
	function api.mark_done(id, on_done)
