...
local function new(domain)
	...
	---@return AtlasForgejoProviderConfig
	function client.config()
		return config.provider_options("forgejo") or {}
	end
	...
	function client.get_auth()
		...
		if base_url == "" or token == "" then
			return "", "Missing Forgejo base_url or token in config"
		end
	...
	function client.request(method, endpoint, data, on_done)
		...
		if auth_err then
			logger.logerror("Forgejo auth missing", { domain = domain, error = auth_err })
		...
		logger.loginfo("Forgejo request", {
			provider = "forgejo",
			domain = domain,
			endpoint = endpoint,
			method = method,
		...
		return http.curl_request(method, client.url(endpoint), client.headers(), payload, function(result, err)
			if err then
				logger.logerror("Forgejo request failed", {
					domain = domain,
					endpoint = endpoint,
					method = method,
					error = err,
